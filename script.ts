interface Book {
  ...
  const sortByName = () => {
    const sortedAuthors = authors.slice().sort((a, b) => nameSorter(a, b));
    displayAuthors(sortedAuthors);
  }
  
  const sortById = () => {
    const sortedAuthors = authors.slice().sort((a, b) => idSorter(a, b));
    displayAuthors(sortedAuthors);
  }
  ...
  const generateAuthorRow = (author: Author) => {
    ...
    biblioCell.addEventListener("click", () => {
        ...
        bookList.innerHTML = "";
        for (const book of author.bibliography.slice().sort((a, b) => a.year - b.year)) {
